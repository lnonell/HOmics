...
- **cont.covar.matrix**: vector or matrix of continuous covariates, with samples as rownames (in the same order as cond) and covariates as columns. Default = NULL
...
The rules to convert the values are the following: if vector it is already numerical, it will be converted to a vector of 0's and 1's using the module 2 function denoted in R with %%.
...
if it is a character variable, it will be converted to a numerical factor.

Remember when converting to factor in R, it assigns the levels in alphabetical order by default. So for instance vector a with 4 character categories, will finally converted to a numerical vector of 0's and 1's this way:
...
The model accepts one or more continuous covariates. The values must be in the same sample order as **cond** and as the columns of **data.matrix**.

# The model

**HOmics** fits a Bayesian hierarchical model for each group of features as specified in the **agg.matrix**. The steps to fit each model are the following:

1. Check and prepare data
2. Depending on the nature of the response variable **cond** and if covariates are included (**cont.covar.matrix**), a different JAGS model is defined. A hierarchical logistic model is fitted if **cond** is a categorical variable whereas a hierarchical linear model is fitted if response is continuous. Each model specifies the likelihood, prior and hyper prior functions and parameters. Models are stored in the /inst directory of the package
3. `r CRANpkg("rjags")` is used to fit the hierarchical model with the MCMC approach
4. The model is initially fitted with three Markov chains and default initial values
5. The model is updated in the *burn in* phase, with 2000 iterations
6. Coefficients (beta estimates) for all features are obtained for the posterior distribution by sampling, with 1000 iterations
7. Function MCMCsummary of package `r CRANpkg("MCMCvis")` is used to obtain summaries
...
**HOmics** objects can be subsequently filtered with function **getsignif()** to get significative features.
...
# Example 3: Synthetic data with an extra continuous covariate
...
res4<-HOmics(data.matrix=data.matrix,
            ...
            cond=cond,
            z.matrix=z.matrix,
            cont.covar.matrix = co.matrix,
            cores=4) 

```

# Example 5: Ovarian cancer gene expression and miRNA integration
...
res5 <- HOmics(data.matrix = expr.m, 
               agg.matrix  = agg.matrix, 
               cond = cond, 
               z.matrix = z.matrix, 
               cont.covar.matrix = NULL, 
...
res5.f.pos <- getsignif(res5, param = "p.pos",threshold = 0.05, as.data.frame = T)
res5.f.pos

res5.f.neg <- getsignif(res5, param = "p.neg",threshold = 0.05, as.data.frame = T)
res5.f.neg
